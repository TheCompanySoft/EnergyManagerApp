...
@interface TimeEnergyChart ()

@property (strong, nonatomic) TimeEnergyLineChart * lineChart;
...
@implementation TimeEnergyChart
...
-(void)setUpChart{
	if (self.chartStyle == UUChartLineStyle) {
        if(!_lineChart){
            _lineChart = [[TimeEnergyLineChart alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
