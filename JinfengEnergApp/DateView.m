...
- (void)clickConfirm
{
    ...
    if (self.delegate &&[self.delegate respondsToSelector:@selector(dateView:)]) {
        [self.delegate  dateView:destDateString];
    }
...
- (void)clickCancle
{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(cancelBtnDelegate)]) {
        [self.delegate  cancelBtnDelegate];
    }
    [self removeFromSuperview];
