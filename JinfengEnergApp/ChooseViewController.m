...
- (void)settabBarButton{
    ...
    [navigationBarView.leftButton addTarget:self action:@selector(rightbackClick) forControlEvents:UIControlEventTouchUpInside];
...
- (void)backClick:(UIButton *)sender{
    if (selectedArray.count>0) {
        ...
        [userDefaults removeObjectForKey:@"selectedCompany"];
        [userDefaults setObject:selectedArray forKey:@"selectedCompany"];
        //这里建议同步存储到磁盘中，但是不是必须的
        [userDefaults synchronize];
        //创建一个消息对象
        NSNotification * notice = [NSNotification notificationWithName:@"selectedCompany" object:@"choice" userInfo:@{@"selectedCompanys":selectedArray}];
        //发送消息
