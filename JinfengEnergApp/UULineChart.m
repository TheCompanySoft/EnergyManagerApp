...
-(void)setYLabels:(NSArray *)yLabels
{
    ...
    for (int i=0; i<7; i++) {
      ...
      label.text = [NSString stringWithFormat:@"%.1f",(_level * i+_yValueMin)];
        if (i==3) {
            UIView *view=[[UIView alloc]initWithFrame:CGRectMake(25*[UIUtils getWindowWidth]/320, chartCavanHeight-i*levelHeight+10*[UIUtils getWindowHeight]/568, self.frame.size.width,0.5)];
    ...
    for (int i=0; i<_yValues.count; i++) {
        ...
        CGPoint point = CGPointMake(xPosition+index*_xLabelWidth, chartCavanHeight - grade * chartCavanHeight+UULabelHeight);
            ...
            [progressline moveToPoint:point];
                ...
                _pointview.layer.borderColor = [[_colors objectAtIndex:i] CGColor]?[[_colors objectAtIndex:i] CGColor]:[UIColor greenColor].CGColor;
                if (isShowMaxAndMinPoint) {
                    _pointview.backgroundColor = [UIColor whiteColor];
                }else{
                    _pointview.backgroundColor = [_colors objectAtIndex:index]?[_colors objectAtIndex:index]:[UIColor colorWithRed:5/255.0 green:251/255.0 blue:250/255.0 alpha:1];
                }
