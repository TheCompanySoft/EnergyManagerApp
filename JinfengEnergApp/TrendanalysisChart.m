...
@property (strong, nonatomic) TrendanalysisLineChart * lineChart;
...
-(void)setUpChart{
	if (self.chartStyle == UUChartLineStyle) {
        if(!_lineChart){
            _lineChart = [[TrendanalysisLineChart alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
            [self addSubview:_lineChart];
        }
 
        //选择显示范围TrendanalysisLineChart
