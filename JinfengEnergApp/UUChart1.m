...
@property (strong, nonatomic) UULineChart1 * lineChart;
...
-(void)setUpChart{
	if (self.chartStyle == UUChartLineStyle) {
        if(!_lineChart){
            _lineChart = [[UULineChart1 alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
